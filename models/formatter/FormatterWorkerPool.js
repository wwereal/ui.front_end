...
export class FormatterWorkerPool {
    ...
    processNextTask() {
        ...
        let freeWorker = [...this.workerTasks.keys()].find(
            (worker) => !this.workerTasks.get(worker)
        );
    ...
    runChunkedTask(
        ...
        function onData(data) {
            ...
            const isLastChunk =
                'isLastChunk' in data && Boolean(data['isLastChunk']);
    ...
    format(mimeType, content, indentString) {
        ...
        return this.runTask(
            'format' /* FormatterActions.FormatterActions.FORMAT */,
            parameters
        );
    }
    javaScriptSubstitute(expression, mapping) {
        return this.runTask(
            'javaScriptSubstitute' /* FormatterActions.FormatterActions.JAVASCRIPT_SUBSTITUTE */,
            { content: expression, mapping }
        ).then((result) => result || '');
    }
    javaScriptScopeTree(expression, sourceType = 'script') {
        return this.runTask(
            'javaScriptScopeTree' /* FormatterActions.FormatterActions.JAVASCRIPT_SCOPE_TREE */,
            { content: expression, sourceType }
        ).then((result) => result || null);
    }
    evaluatableJavaScriptSubstring(content) {
        return this.runTask(
            'evaluatableJavaScriptSubstring' /* FormatterActions.FormatterActions.EVALUATE_JAVASCRIPT_SUBSTRING */,
            { content: content }
        ).then((text) => text || '');
    }
    parseCSS(content, callback) {
        this.runChunkedTask(
            'parseCSS' /* FormatterActions.FormatterActions.PARSE_CSS */,
            { content: content },
            onDataChunk
        );
        // TODO(crbug.com/1172300) Ignored during the jsdoc to ts migration
