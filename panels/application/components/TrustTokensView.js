...
const UIStrings = {
     ...
     */
    allStoredTrustTokensAvailableIn:
        'All stored private state tokens available in this browser instance.',
    /**
     ...
     */
    deleteTrustTokens:
        'Delete all stored private state tokens issued by {PH1}.',
    /**
...
const str_ = i18n.i18n.registerUIStrings(
    'panels/application/components/TrustTokensView.ts',
    UIStrings
);
export const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);
const coordinator =
    RenderCoordinator.RenderCoordinator.RenderCoordinator.instance();
/** Fetch the Trust Token data regularly from the backend while the panel is open */
const REFRESH_INTERVAL_MS = 1000;
export class TrustTokensView extends LegacyWrapper.LegacyWrapper
    .WrappableComponent {
    ...
    #deleteClickHandler(issuerOrigin) {
        ...
        void mainTarget
            ?.storageAgent()
            .invoke_clearTrustTokens({ issuerOrigin });
    }
    connectedCallback() {
    ...
    async render() {
        ...
        const { tokens } = await mainTarget
            .storageAgent()
            .invoke_getTrustTokens();
        ...
        <div>
          ...
          <${IconButton.Icon.Icon.litTagName} name="info" title=${i18nString(
                    UIStrings.allStoredTrustTokensAvailableIn
    ...
    #renderGridOrNoDataMessage(tokens = []) {
        if (tokens.length === 0) {
            return LitHtml.html`<div class="no-tt-message">${i18nString(
                UIStrings.noTrustTokensStored
            )}</div>`;
        }
        const gridData = {
            columns: [
                ...
                cells: [
                    {
                        ...
                        renderer:
                            this.#deleteButtonRendererForDataGridCell.bind(
                                this
                            ),
                    },
                    {
    ...
    #deleteButtonRendererForDataGridCell(issuer) {
      ...
      <${Buttons.Button.Button.litTagName} .iconName=${'bin'}
                                           ...
                                           .size=${
                                               'SMALL' /* Buttons.Button.Size.SMALL */
                                           }
                                           .title=${i18nString(
                                               UIStrings.deleteTrustTokens,
                                               { PH1: issuer }
                                           )}
                                           .variant=${
                                               'icon' /* Buttons.Button.Variant.ICON */
                                           }
                                           @click=${this.#deleteClickHandler.bind(
                                               this,
                                               issuer
                                           )}></${
