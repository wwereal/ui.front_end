...
const str_ = i18n.i18n.registerUIStrings(
    'panels/network/RequestInitiatorView.ts',
    UIStrings
);
...
export class RequestInitiatorView extends UI.Widget.VBox {
    ...
    constructor(request) {
        ...
        this.element.setAttribute(
            'jslog',
            `${VisualLogging.pane('initiator').track({ resize: true })}`
        );
        this.linkifier = new Components.Linkifier.Linkifier();
        this.request = request;
        this.emptyWidget = new UI.EmptyWidget.EmptyWidget(
            i18nString(UIStrings.thisRequestHasNoInitiatorData)
        );
    ...
    createTree() {
        ...
        treeOutline.contentElement.setAttribute(
            'jslog',
            `${VisualLogging.tree('initiator-tree')}`
        );
        return treeOutline;
    }
    buildRequestChainTree(initiatorGraph, title, tree) {
        ...
        if (root.titleElement instanceof HTMLElement) {
            root.titleElement.classList.add(
        ...
        for (const request of initiated.keys()) {
            if (initiated.get(request) === parentRequest) {
                ...
                if (!visited.has(request)) {
                    visited.add(request);
                    this.depthFirstSearchTreeBuilder(
                        initiated,
                        treeElement,
                        request
                    );
    ...
    wasShown() {
        ...
        const stackTracePreview = RequestInitiatorView.createStackTracePreview(
            this.request,
            this.linkifier,
            true
        );
        ...
        if (
            initiatorGraph.initiators.size > 1 ||
            initiatorGraph.initiated.size > 1
        ) {
            initiatorDataPresent = true;
            this.buildRequestChainTree(
                initiatorGraph,
