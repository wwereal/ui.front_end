    ...
    return fmt.replaceAll(RE, (_, index, precision, specifier) => {
        ...
        if (argIndex >= args.length) {
            throw new RangeError(
                `Expected at least ${
                    argIndex + 1
                } format parameters, but only ${args.length} where given.`
            );
        }
        if (specifier === 's') {
        ...
        if (specifier === 'd') {
            return String(Math.floor(argValue)).padStart(
                Number(precision),
                '0'
            );
        }
...
const EXTENDED_KEBAB_CASE_REGEXP =
...
const WORD =
    /[A-Z]{2,}(?=[A-Z0-9][a-z0-9]+|\b|_)|[A-Za-z][0-9]+[a-z]?|[A-Z]?[a-z]+|[0-9][A-Za-z]+|[A-Z]|[0-9]+|[.]/g;
//            <---1---><------------2-----------> <---------3--------> <-----4----> <------5-----> <-----6----> <7>
...
export const replaceLast = function (input, search, replacement) {
    ...
    return (
        input.slice(0, replacementStartIndex) +
        input.slice(replacementStartIndex).replace(search, replacement)
    );
};
export const stringifyWithPrecision = function stringifyWithPrecision(
    s,
    precision = 2
) {
    if (precision === 0) {
