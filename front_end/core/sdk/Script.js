...
export class Script {
    ...
    isJavaScript() {
        return (
            this.#language ===
            'JavaScript' /* Protocol.Debugger.ScriptLanguage.JavaScript */
        );
    }
    isWasm() {
        return (
            this.#language ===
            'WebAssembly' /* Protocol.Debugger.ScriptLanguage.WebAssembly */
        );
    }
    scriptLanguage() {
    ...
    async loadTextContent() {
        const result = await this.debuggerModel
            .target()
            .debuggerAgent()
        ...
        if (
            this.hasSourceURL &&
            Common.ParsedURL.schemeIs(this.sourceURL, 'snippet:')
        ) {
    ...
    async loadWasmContent() {
        ...
        if (streamId) {
            while (true) {
                const result = await this.debuggerModel
                    .target()
                    .debuggerAgent()
                    .invoke_nextWasmDisassemblyChunk({ streamId });
                if (result.getError()) {
                    throw new Error(result.getError());
                }
                const {
                    chunk: {
                        lines: linesChunk,
                        bytecodeOffsets: bytecodeOffsetsChunk,
                    },
                } = result;
                totalLength += linesChunk.reduce(
        ...
        for (let i = 0; i < functionBodyOffsets.length; i += 2) {
            functionBodyRanges.push({
                start: functionBodyOffsets[i],
                end: functionBodyOffsets[i + 1],
            });
        }
    ...
    requestContentData() {
        if (!this.#contentPromise) {
            ...
            if (
                this.hash &&
                !this.#isLiveEditInternal &&
                this.contentLength > fileSizeToCache
            ) {
                ...
                if (!scriptCacheInstance) {
                    ...
                    scriptCacheInstance = {
                        cache: new Map(),
                        registry: new FinalizationRegistry((hashCode) =>
                            scriptCacheInstance?.cache.delete(hashCode)
                        ),
                    };
                }
                ...
                const cachedContentPromise = scriptCacheInstance.cache
                    .get(fullHash)
                    ?.deref();
                if (cachedContentPromise) {
                    ...
                    scriptCacheInstance.cache.set(
                        fullHash,
                        new WeakRef(this.#contentPromise)
                    );
                    scriptCacheInstance.registry.register(
                        this.#contentPromise,
                        fullHash
                    );
                }
    ...
    async getWasmBytecode() {
        const base64 = await this.debuggerModel
            .target()
            .debuggerAgent()
            .invoke_getWasmBytecode({ scriptId: this.scriptId });
        const response = await fetch(
            `data:application/wasm;base64,${base64.bytecode}`
        );
    ...
    async searchInContent(query, caseSensitive, isRegex) {
        ...
        const matches = await this.debuggerModel
            .target()
            .debuggerAgent()
        ...
        return TextUtils.TextUtils.performSearchInSearchMatches(
            matches.result || [],
            query,
            caseSensitive,
            isRegex
    ...
    async editSource(newSource) {
        ...
        const oldSource = TextUtils.ContentData.ContentData.textOr(
            await this.requestContentData(),
            null
        );
        ...
        const response = await this.debuggerModel
            .target()
            .debuggerAgent()
        ...
        if (response.getError()) {
            ...
            throw new Error(
                `Script#editSource failed for script with id ${
                    this.scriptId
                }: ${response.getError()}`
            );
        }
        if (
            !response.getError() &&
            response.status ===
                'Ok' /* Protocol.Debugger.SetScriptSourceResponseStatus.Ok */
        ) {
        ...
        this.debuggerModel.dispatchEventToListeners(
            Events.ScriptSourceWasEdited,
            { script: this, status: response.status }
        );
        return {
            changed: true,
    ...
    async setBlackboxedRanges(positions) {
        const response = await this.debuggerModel
            .target()
            .debuggerAgent()
            .invoke_setBlackboxedRanges({ scriptId: this.scriptId, positions });
    ...
    get isBreakpointCondition() {
        return [COND_BREAKPOINT_SOURCE_URL, LOGPOINT_SOURCE_URL].includes(
            this.sourceURL
        );
    }
    /**
...
function frameIdForScript(script) {
    ...
    const resourceTreeModel = script.debuggerModel
        .target()
...
async function disassembleWasm(content) {
    const worker = Common.Worker.WorkerWrapper.fromWorker(
        new Worker(
            new URL(
                ...
                switch (data.method) {
                    case 'disassemble':
                        if ('error' in data) {
                            ...
                            const { lines, offsets, functionBodyOffsets } =
                                data.result;
                            resolve(
                                new TextUtils.WasmDisassembly.WasmDisassembly(
                                    lines,
                                    offsets,
                                    functionBodyOffsets
                                )
