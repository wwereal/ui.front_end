...
self.onmessage = function (event) {
    ...
    switch (method) {
        case 'format' /* FormatterActions.FORMAT */:
            self.postMessage(
                FormatterWorker.FormatterWorker.format(
                    params.mimeType,
                    params.content,
                    params.indentString
                )
        ...
        case 'parseCSS' /* FormatterActions.PARSE_CSS */:
            FormatterWorker.CSSRuleParser.parseCSS(
                params.content,
                self.postMessage
            );
            break;
        case 'javaScriptSubstitute' /* FormatterActions.JAVASCRIPT_SUBSTITUTE */: {
            self.postMessage(
                FormatterWorker.Substitute.substituteExpression(
                    params.content,
                    params.mapping
                )
            );
        ...
        case 'javaScriptScopeTree' /* FormatterActions.JAVASCRIPT_SCOPE_TREE */: {
            self.postMessage(
                FormatterWorker.ScopeParser.parseScopes(
                    params.content,
                    params.sourceType
                )?.export()
            );
        ...
        case 'evaluatableJavaScriptSubstring' /* FormatterActions.EVALUATE_JAVASCRIPT_SUBSTRING */:
            self.postMessage(
                FormatterWorker.FormatterWorker.evaluatableJavaScriptSubstring(
                    params.content
                )
            );
            break;
